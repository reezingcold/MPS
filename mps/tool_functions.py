...
# as well as cut_off condition
def svd_bdc(mat: np.ndarray, bdc: int = 8, cut_off: float = 1.e-8)->tuple:
    '''
    @mat: matrix to be decomposed
    @bdc: bond dimension constraints
    @cut_off: eigenvals smaller than cut_off will be removed
    '''
    U, Svals, V = linalg.svd(mat)
    trunc = min(bdc, len(Svals))
    for i in range(0, len(Svals)):
        if abs(Svals[i]) < cut_off:
            trunc = min(trunc, i)
            break
...
# much more faster than np.einsum
