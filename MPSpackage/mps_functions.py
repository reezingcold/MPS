...
def operate(operator: MPO, state: MPS)->MPS:
    ...
    for s in range(0, N):
        ...
        if s == 0:
            new_site_tensor = np.einsum('ijk,kb->jib', site_operator, site_tensor)
            j, i, b = new_site_tensor.shape
            new_site_tensor = np.reshape(new_site_tensor, (j, i*b))
        elif s == N-1:
            new_site_tensor = np.einsum('ijk,ak->iaj', site_operator, site_tensor)
            i, a, j = new_site_tensor.shape
            new_site_tensor = np.reshape(new_site_tensor, (i*a, j))
        else:
            new_site_tensor = np.einsum('ijkl,alc->iakjc', site_operator, site_tensor)
...
def multiply(phi_dag: MPS, psi: MPS)->complex:
    ...
    for s in range(0, N):
        ...
        if s != 0 and s != N-1:
            site_matrix = np.einsum('def,aec->dafc', site_phi, site_psi)
            d, a, f, c = site_matrix.shape
            site_matrix = np.reshape(site_matrix, (d*a, f*c))
        elif s == 0:
            site_matrix = np.einsum('cd,cb->db', site_phi, site_psi)
            d, b = site_matrix.shape
            site_matrix = np.reshape(site_matrix, (1, d*b))
        else:
            site_matrix = np.einsum('cd,ad->ca', site_phi, site_psi)
...
def site_multiply(phi_dag: MPS, psi: MPS, site: int)->np.ndarray:
    ...
    if site != 0 and site != N-1:
        site_tensor = np.einsum('def,aec->dafc', tensor_up, tensor_down)
        d, a, f, c = site_tensor.shape
        site_tensor = np.reshape(site_tensor, (d*a, f*c))
    elif site == 0:
        site_tensor = np.einsum('cd,cb->db', tensor_up, tensor_down)
        d, b = site_tensor.shape
        site_tensor = np.reshape(site_tensor, (1, d*b))
    else:
        site_tensor = np.einsum('cd,ad->ca', tensor_up, tensor_down)
...
def contract(phi_dag: MPS, psi: MPS, site: int):
    ...
    if site != 0 and site != N-1:
        ...
        vec = np.reshape(np.einsum('ha,abc,jc->hbj', rhs, site_psi, lhs), (1, d*b*f))
    elif site == 0:
        rhs = matprod([site_multiply(phi_dag, psi, i) for i in range(1, N)])
        a, b = site_psi.shape
        c, d = site_phi.shape
        rhs = np.reshape(rhs, (d, b))
        vec = np.reshape(np.einsum('ab,eb->ae', site_psi, rhs), (1, a*d))
    else:
        lhs = matprod([site_multiply(phi_dag, psi, i) for i in range(0, N-1)])
        a, b = site_psi.shape
        c, d = site_phi.shape
        lhs = np.reshape(lhs, (c, a))
        vec = np.reshape(np.einsum('ea,ab->eb', lhs, site_psi), (1, c*b))
    return vec
...
def mps_contract(phi: MPS)->np.ndarray:
    ...
    for i in range(1, N-1):
        a, b = vec.shape
        b, c, d = phi.get_dim(i)
        vec = np.reshape(np.einsum('ab,bcd->acd', vec, phi.get_data(i)), (a*c, d))
    a, c = vec.shape[0], phi.get_dim(N-1)[1]
    vec = np.reshape(np.einsum('ab,bc->ac', vec, phi.get_data(N-1)), (a*c, 1))
    return vec
...
def mpo_contract(oper: MPO)->np.ndarray:
    ...
    for i in range(1, N-1):
        ...
        mat = np.reshape(np.einsum('abc,aklm->kblcm', mat, oper.get_data(i)), (k, b*l, c*m))
    a, b, c = mat.shape
    d, e, f = oper.get_dim(N-1)
    mat = np.reshape(np.einsum('abc,aef->becf', mat, oper.get_data(N-1)), (b*e, c*f))
    return mat
