...
def operate(operator: MPO, state: MPS)->MPS:
    ...
    result_state.update_bonddim()
    result_state.bond_D = max(result_state.bond_dims)
    return result_state
...
def accurate_plus(phi: MPS, psi: MPS)->MPS:
    ...
    result = MPS(N, phy_dims = phi.phy_dims, bond_D = max(phi.bond_dims)+max(psi.bond_dims), initial = 'z')
    # site == 0
...
def getEntropy(mps: MPS, bond_site: int):
    mps_copy = mps.copy()
    mps_copy.normalize(bond_site)
    site_tensor, next_tensor = mps_copy.get_data(bond_site), mps_copy.get_data(bond_site+1)
    del mps_copy
    a, b, c = site_tensor.shape
    c, d, e = next_tensor.shape
    site_matrix, next_matrix = np.reshape(site_tensor, (a*b, c)), np.reshape(next_tensor, (c, d*e))
    Svals = linalg.svdvals(site_matrix@next_matrix)
    entropy = 0
    for s in Svals:
        entropy += -s**2*np.log2(s**2) if s > 0 else 0
    return entropy

# calculate <psi|O|psi>
def expect(O: MPO, psi: MPS, output_real: bool = False)->complex:
    return multiply(psi.dag(), operate(O, psi)) if not output_real else abs(multiply(psi.dag(), operate(O, psi)))